    ...
    try {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            try {
                ...
                await page.waitForSelector('.emailbox-input', { visible: true, timeout: 30000 });
                await page.waitForFunction(
                    () => (document.querySelector('.emailbox-input') as HTMLInputElement)?.value.includes('@'),
                    { timeout: 30000 }
                );
                // Get the email from the input box
                const email = await page.evaluate(() => {
                    return (document.querySelector('.emailbox-input') as HTMLInputElement)?.value;
                });
